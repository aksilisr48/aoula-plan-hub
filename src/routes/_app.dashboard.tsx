...
function DashboardPage() {
  ...
  const blocsPlanifies = blocs.filter(
    (b) => b.status === "Planifié" || b.status === "Terminé",
  ).length;
  const blocsAPlanifier = blocs.filter(
    (b) => b.status === "Non planifié" || b.status === "Incomplet",
  ).length;
  const demandesAttente = demandes.filter(
  ...
  const kpis = [
    ...
    {
      label: "Blocs à planifier",
      value: blocsAPlanifier,
      icon: CalendarRange,
      hint: "Non encore programmés",
      trend: null,
    },
    {
      label: "Blocs planifiés",
      value: blocsPlanifies,
      icon: CheckCircle2,
      hint: "Programmés ou terminés",
      trend: `${avancement}% du total`,
    },
    {
      label: "Demandes MM",
      value: demandesAttente,
      icon: Truck,
          ...
          <>
            ...
            <Link to="/planification">
              <Button className="gap-2 shadow-soft">
                <CalendarPlus className="h-4 w-4" /> Planifier un bloc
              </Button>
            </Link>
          </>
        }
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        {kpis.map((k) => {
          ...
          return (
            <Card
              key={k.label}
              className="p-5 border-border bg-surface shadow-soft hover:shadow-elevated transition-shadow"
            >
              ...
              <div className="mt-4 text-2xl font-bold text-[color:var(--brand-night)] tracking-tight">
                {k.value}
              </div>
          ...
          <div className="flex items-start justify-between mb-5">
            <div>
              <h3 className="font-semibold text-[color:var(--brand-night)]">
                Activité de planification — 7 derniers jours
              </h3>
              <p className="text-xs text-muted-foreground mt-0.5">
                Blocs planifiés et blocs terminés
              </p>
            </div>
            ...
            <ResponsiveContainer>
              <AreaChart data={ACTIVITY_DATA} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <XAxis
                  ...
                  tickLine={false}
                  axisLine={false}
                  tick={{ fontSize: 12, fill: "oklch(0.52 0.04 257)" }}
                />
                <YAxis
                  tickLine={false}
                  axisLine={false}
        ...
        <Card className="lg:col-span-2 p-6 border-border bg-surface shadow-soft">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Dernières émissions enregistrées sur Al Aoula
              </p>
            </div>
            <Link
              to="/emissions"
      ...
      <Card className="p-6 border-border bg-surface shadow-soft">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Prochaines actions de planification
            </p>
          </div>
          <Link
            to="/planification"
