...
function SuiviPage() {
  ...
  const enAttenteMM = demandes.filter((d) => d.status === "Demandée" || d.status === "En attente").length;
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <KPI tone="primary" icon={CheckCircle2} value={`${totalBlocs ? Math.round((planifies/totalBlocs)*100) : 0}%`} label="Avancement global" />
        <KPI tone="success" icon={CheckCircle2} value={planifies} label="Blocs planifiés / terminés" />
              ...
              <div className="flex flex-col lg:flex-row lg:items-start gap-5">
                <div className="flex-1">
                  <Link to="/productions/$id" params={{ id: p.id }} className="group">
                    <div className="text-xs text-muted-foreground">{emi?.title} · <span className="font-mono">{p.code}</span></div>
                    <h3 className="mt-1 font-semibold text-[color:var(--brand-night)] group-hover:text-primary transition-colors">{p.name}</h3>
                  </Link>
                  <div className="mt-3 flex items-center gap-3 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> {p.dateStart} → {p.dateEnd}</span>
                  ...
                  <div className="mt-5 grid grid-cols-2 md:grid-cols-4 gap-2">
                    {PHASES.map((phase) => {
                      ...
                      const phDone = ph.filter((b) => b.status === "Planifié" || b.status === "Terminé").length;
                      const phPct = ph.length ? Math.round((phDone/ph.length)*100) : 0;
                      return (
                        <div key={phase} className="rounded-lg border border-border p-2.5 bg-muted/30">
                          <div className="text-[11px] text-muted-foreground">{phase}</div>
                          <div className="mt-0.5 text-sm font-semibold text-[color:var(--brand-deep)] tabular-nums">{phDone}/{ph.length}</div>
                ...
                <div className="lg:w-72 space-y-2">
                  {inc > 0 && (
                    <div className="rounded-lg border border-destructive/20 bg-destructive/10 p-3 text-xs">
                      <div className="flex items-center gap-1.5 font-medium text-destructive"><AlertTriangle className="h-3.5 w-3.5" /> {inc} bloc(s) incomplet(s)</div>
                      <div className="text-muted-foreground mt-0.5">Compléter les champs manquants.</div>
                    </div>
                  )}
                  {demandesP.length > 0 && (
                    <div className="rounded-lg border border-border bg-muted/30 p-3 text-xs">
                      <div className="flex items-center gap-1.5 font-medium"><Truck className="h-3.5 w-3.5 text-primary" /> Demandes MM</div>
                      <div className="mt-1 space-y-1">
                        {demandesP.map((d) => (
                          <div key={d.id} className="flex items-center justify-between gap-2">
                            <span className="truncate text-muted-foreground">{d.typeCouverture}</span>
                  ...
                  {inc === 0 && demandesP.length === 0 && (
                    <div className="rounded-lg border border-success/20 bg-success/10 p-3 text-xs">
                      <div className="flex items-center gap-1.5 font-medium text-[color:oklch(0.4_0.15_145)]"><CheckCircle2 className="h-3.5 w-3.5" /> Aucune alerte</div>
                      <div className="text-muted-foreground mt-0.5">La production est dans les délais.</div>
                    </div>
                  )}
                </div>
              </div>
...
function KPI({ value, label, tone, icon: Icon }: { value: number | string; label: string; tone: "primary" | "success" | "warning" | "info"; icon: React.ComponentType<{ className?: string }> }) {
  const toneClass = {
  ...
  return (
    <Card className="p-5 border-border bg-surface shadow-soft">
      ...
      <div className="mt-4 text-2xl font-bold text-[color:var(--brand-night)] tabular-nums">{value}</div>
