...
function LoginPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!email || !password) {
      toast.error("Identifiants incorrects", { description: "Veuillez renseigner votre login et mot de passe." });
      return;
    }
    setLoading(true);
    setTimeout(() => {
      login(email);
      toast.success("Connexion réussie", { description: "Bienvenue dans votre espace de travail." });
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <Button type="submit" className="h-11 w-full gap-2 text-[15px] shadow-soft" disabled={loading}>
        ...
        <div className="flex items-start gap-2 rounded-xl bg-accent/50 border border-border p-3 text-xs text-muted-foreground">
          ...
          <div>
            Accès réservé aux collaborateurs SNRT. Toute connexion est tracée pour des raisons de sécurité.
          </div>
        </div>
      </form>
    </AuthShell>
