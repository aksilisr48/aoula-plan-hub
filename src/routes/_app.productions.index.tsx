    ...
    <div>
      ...
      {productions.length === 0 ? (
        <Card className="p-12 border-dashed border-2 bg-accent/20 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">Créez une production depuis la fiche d'une émission.</p>
        </Card>
      ) : (
              ...
              <Link key={p.id} to="/productions/$id" params={{ id: p.id }}>
                <Card className="p-5 border-border bg-surface shadow-soft hover:shadow-elevated hover:border-primary/40 transition-all h-full">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="text-xs font-mono px-2 py-0.5 rounded bg-muted text-muted-foreground inline-block">{p.code}</div>
                      <h3 className="mt-2 font-semibold text-[color:var(--brand-night)] leading-tight">{p.name}</h3>
                  ...
                  <div className="mt-4 pt-4 border-t border-border">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-medium tabular-nums">{done}/{list.length} blocs · {pct}%</span>
                    </div>
