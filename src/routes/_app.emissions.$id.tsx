...
import { ArrowLeft, Plus, Clapperboard, Calendar, MapPin, User as UserIcon, ListChecks } from "lucide-react";
...
function EmissionDetailPage() {
  ...
  if (!emission) {
    return (
      <div className="text-sm text-muted-foreground">
        Émission introuvable. <Link to="/emissions" className="text-primary">Retour à la liste</Link>
      </div>
    );
  }
  ...
  function createProd() {
    ...
    setOpen(false);
    toast.success("Production créée", { description: `${prod.name} héritera du modèle ${emission!.model} avec ses blocs opérationnels.` });
  ...
  return (
    <div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
          <div className="flex gap-5">
            <div className="h-16 w-16 rounded-2xl bg-gradient-to-br from-primary-soft to-accent grid place-items-center text-[color:var(--brand-deep)] font-bold text-lg shrink-0">
              {emission.title.split(" ").map((w) => w[0]).slice(0, 2).join("")}
            </div>
            <div>
              <div className="text-xs font-semibold uppercase tracking-wider text-primary">Fiche émission</div>
              <h1 className="text-2xl font-bold text-[color:var(--brand-night)] tracking-tight mt-1">{emission.title}</h1>
              <div className="mt-2 flex flex-wrap items-center gap-2">
                <span className="text-xs px-2 py-1 rounded-md bg-accent/60 text-[color:var(--brand-deep)] font-medium">{emission.category}</span>
      ...
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-lg font-semibold text-[color:var(--brand-night)]">Productions associées</h2>
      </div>
      ...
      {productions.length === 0 ? (
        <Card className="p-12 border-dashed border-2 bg-accent/20 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">Créez une première production. Elle héritera automatiquement du modèle <span className="font-medium text-[color:var(--brand-deep)]">{emission.model}</span>.</p>
          <Button className="mt-4 shadow-soft" onClick={() => setOpen(true)}><Plus className="h-4 w-4" /> Ajouter production</Button>
        </Card>
      ) : (
            ...
            <Link key={p.id} to="/productions/$id" params={{ id: p.id }}>
              <Card className="p-5 border-border bg-surface shadow-soft hover:shadow-elevated hover:border-primary/40 transition-all cursor-pointer h-full">
                <div className="flex items-start justify-between">
                  <span className="text-xs font-mono px-2 py-0.5 rounded bg-muted text-muted-foreground">{p.code}</span>
                  <StatusBadge value={p.priority} />
                </div>
                <h3 className="mt-3 font-semibold text-[color:var(--brand-night)]">{p.name}</h3>
                <div className="mt-2 space-y-1 text-xs text-muted-foreground">
                  <div className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5" /> {p.dateStart} → {p.dateEnd}</div>
                  <div className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" /> {p.lieu}</div>
                  <div className="flex items-center gap-1.5"><UserIcon className="h-3.5 w-3.5" /> {p.responsable}</div>
                </div>
              </Card>
            </Link>
          ))}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              La production héritera automatiquement du modèle <span className="font-medium text-[color:var(--brand-deep)]">{emission.model}</span> avec ses phases et blocs opérationnels.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2 space-y-2">
              ...
              <Input placeholder="Ex. JT 20h30 — Édition 30 mai" value={pName} onChange={(e) => setPName(e.target.value)} />
            </div>
            <div className="space-y-2"><Label>Date début *</Label><Input type="date" value={pStart} onChange={(e) => setPStart(e.target.value)} /></div>
            <div className="space-y-2"><Label>Date fin *</Label><Input type="date" value={pEnd} onChange={(e) => setPEnd(e.target.value)} /></div>
            <div className="space-y-2"><Label>Lieu principal</Label><Input placeholder="Studio A — SNRT Rabat" value={pLieu} onChange={(e) => setPLieu(e.target.value)} /></div>
            <div className="space-y-2"><Label>Responsable *</Label><Input placeholder="Ex. M. El Idrissi" value={pResp} onChange={(e) => setPResp(e.target.value)} /></div>
            ...
            <div className="sm:col-span-2 space-y-2">
              ...
              <Textarea rows={3} value={pCom} onChange={(e) => setPCom(e.target.value)} placeholder="Notes opérationnelles, contraintes, instructions…" />
            </div>
          </div>
