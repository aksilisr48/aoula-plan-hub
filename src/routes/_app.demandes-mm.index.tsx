              ...
              >
                <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap gap-3 text-xs text-muted-foreground">
                        ...
                        <span className="flex items-center gap-1">
                          <Clock className="h-3.5 w-3.5" /> {d.heure}
                        </span>
                        <span className="flex items-center gap-1">
                          <MapPin className="h-3.5 w-3.5" /> {d.lieu}
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-1.5 lg:max-w-sm lg:justify-end">
                    {d.ressources.map((r) => (
                      <span
                        key={r}
                        className="text-[11px] px-2 py-1 rounded-md bg-accent/60 text-[color:var(--brand-deep)]"
                      >
                        {r}
                      </span>
                    ))}
                ...
                {d.justification && (
                  <div className="mt-3 pt-3 border-t border-border text-xs text-muted-foreground">
                    <span className="font-medium text-foreground/70">Justification : </span>
                    {d.justification}
                  </div>
                )}
              </Card>
...
function KPI({
  ...
  return (
    <Card className="p-5 border-border bg-surface shadow-soft">
      <div className={`text-2xl font-bold tabular-nums ${toneClass.split(" ").slice(1).join(" ")}`}>
        {value}
      </div>
