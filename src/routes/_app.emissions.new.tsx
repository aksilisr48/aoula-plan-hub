...
function NewEmissionPage() {
  ...
  function submit(e: React.FormEvent) {
    ...
    toast.success("Émission créée", {
      description: `${em.title} a été enregistrée avec son modèle de production.`,
    });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <Card className="lg:col-span-2 p-6 border-border bg-surface shadow-soft space-y-5">
            <div className="space-y-2">
              <Label htmlFor="title">
                Titre de l'émission <span className="text-destructive">*</span>
              </Label>
              <Input
                id="title"
                placeholder="Ex. JT Al Aoula 20h30"
                value={title}
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="desc"
                rows={4}
                placeholder="Décrivez brièvement l'émission, sa ligne éditoriale, son public cible…"
                value={description}
          ...
          <Card className="p-6 border-border bg-gradient-to-br from-primary-soft to-accent/40 shadow-soft h-fit">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Selon la catégorie choisie, mPlanner associe automatiquement un modèle de production
            ...
            <ul className="mt-4 text-xs text-muted-foreground space-y-1.5">
              <li className="flex gap-2">
                <span className="text-primary">•</span> Pré-production
              </li>
              <li className="flex gap-2">
                <span className="text-primary">•</span> Production
              </li>
              <li className="flex gap-2">
                <span className="text-primary">•</span> Post-production
              </li>
              <li className="flex gap-2">
                <span className="text-primary">•</span> Diffusion
              </li>
            </ul>
          </Card>
        </div>
